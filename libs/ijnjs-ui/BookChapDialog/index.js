...
(function (root) {
    ...
    // document ready
    $(() => {
        var html = caches.getStr("BookChapDialog/BookChapDialog.html")
        var Enumerable = Ijnjs.Libs.s.libs.Enumerable
        var r3 = Enumerable.from($(html)).firstOrDefault(a1 => $(a1).attr("id") == 'book-chap-dialog')
        $(r3.outerHTML).appendTo('body')

        function aaa() { eval(caches.getStr("BookChapDialog/BookChapDialog")) }
        var tmp = {}
        aaa.call(tmp)
    ...
    function releaseCacheHere() {
        var Enumerable = Ijnjs.Libs.s.libs.Enumerable
        var caches = Ijnjs.cachesUI
        Enumerable.from(['index', 'BookChapDialog', 'BookChapDialog.html'])
            .select(a1 => 'BookChapDialog/' + a1)
