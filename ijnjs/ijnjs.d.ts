declare namespace Ijnjs {
  export class TestTime {
    /**
     * 
     * @param isLog 當測試完後，常常是把所有 code 拿掉, 用這變數可以讓 code 留著. 但在 log 時直接 return
     */
    constructor(isLog: boolean)
    log(msg: string, isReset: boolean)
  }
  /**
   * 取代 原本的 script scr，所以是 sync 的
   * @param url ajax 的 url 參數
   */
  ...
  export function appendObjectTo(objSrc: Object, objDst: Object)
   ...
   * window.width < 36rem
   */
  export function isLessMidWindow(): boolean
  /**
   ...
   * @returns 
   */
  export function getSrd(dir: string): string
  /**
   * 
   * @param path 相對於 index.html 的路徑，在前面會加 /NUI/ 或 /NUII/，例如傳入 'ijnjs/ijnjs.js'
   * @param isCache true 或 false
   */
  export function loadJsInIIFEModel(path: string, isCache: boolean): Promise<Object>
